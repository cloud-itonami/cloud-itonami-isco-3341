(ns officesupervision.actor
  "OfficeSupervisionActor — the ISCO-08 3341 community office
  supervision actor as a `langgraph.graph/state-graph`
  (ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  office-coordination operation request (intake → advise → govern →
  ...
  The unconditional invariant: the Office Supervision Advisor can
  never directly commit a record the OfficeSupervisionGovernor
  refuses, and can never finalize a disciplinary action, termination
  or performance-review determination — every commit-record! call is
  gated behind `:decide`."
...
(defn build-graph
  "Build a compiled OfficeSupervisionActor graph. `store` implements
  `officesupervision.store/Store`. `advisor` implements
  `officesupervision.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:office-id (:office-id request)
                                    :op (:op proposal)
                                    :staff-id (:staff-id proposal)
                                    :workflow-id (:workflow-id proposal)
                                    :payload proposal}]
